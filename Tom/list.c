...
// TODO add size
LIST *list_new_item(char *newfname, char *newfhash, int size)
{
...
LIST *list_add(LIST *list, char *newfname, char *newfhash, int size)
{   
    ...
    if(list_find(list, newfname, newfhash)) {            // only add each item once
        ...
        LIST *new   = list_new_item(newfname, newfhash, size);
        new->next   = list;
        return new;
    }
}
...
void list_print(LIST *list)
{
    if(list != NULL) {
        while(list != NULL) {
	    printf("%s-%i-%s", list->fname, list->size, list->fhash);
	    if(list->next != NULL) {
	        printf(" -> ");
            }
	    list	= list->next;
